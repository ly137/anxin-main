...
public class PathDetailPanel extends JPanel {
    ...
    public PathDetailPanel() {
        ...
        JButton exportButton = UiStyles.ghostButton("导出路线");
        exportButton.addActionListener(e -> exportRoute());
        JPanel headerEast = new JPanel(new FlowLayout(FlowLayout.RIGHT, 8, 0));
        headerEast.setOpaque(false);
        headerEast.add(exportButton);
        headerEast.add(statusBadgeLabel);

        summaryHeader.add(summaryTitleGroup, BorderLayout.CENTER);
        summaryHeader.add(headerEast, BorderLayout.EAST);
    ...
    private void exportRoute() {
        if (rawContent == null || rawContent.trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "暂无可导出的路线，请先完成一次路径查询。", "提示", JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        JFileChooser chooser = new JFileChooser();
        chooser.setDialogTitle("导出路线");
        chooser.setFileFilter(new FileNameExtensionFilter("文本文件 (*.txt)", "txt"));
        chooser.setSelectedFile(new File("route.txt"));
        ...
        try {
            ...
            writer.print(rawContent);
            writer.close();
            JOptionPane.showMessageDialog(this, "路线已导出到: " + file.getAbsolutePath(), "导出成功", JOptionPane.INFORMATION_MESSAGE);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "导出失败: " + ex.getMessage(), "导出错误", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private static final class RouteSummaryData {
        ...
        private static RouteSummaryData fromContent(String content) {
            ...
            for (String line : lines) {
                ...
                if (trimmed.startsWith("出行方式：")) {
                    transportLabel = trimmed.substring("出行方式：".length()).trim();
                    continue;
                }
                ...
                if (trimmed.startsWith("预计") && trimmed.contains("耗时：")) {
                    int idx = trimmed.indexOf("耗时：");
                    time = trimmed.substring(idx + "耗时：".length()).trim();
                    continue;
                }
        ...
        private String headlineText() {
            ...
            String base = startName + "  →  " + endName;
            if (transportLabel != null && !transportLabel.isEmpty()) {
                base = "[" + transportLabel + "] " + base;
            }
            return base;
        }
