...
public class PathQueryView extends JPanel {
    private static final int HEADER_GAP = 16;
    private static final int FIELD_CARD_GAP = 8;
    private static final int FIELD_CARD_PADDING_Y = 12;
    private static final int FIELD_CARD_PADDING_X = 14;
    private static final int FIELD_STACK_GAP = 10;
    ...
    private final JButton walkingButton;
    private final JButton shuttleBusButton;
    ...
    public PathQueryView() {
        ...
        walkingButton = createTransportModeButton("步行", true);
        shuttleBusButton = createTransportModeButton("摆渡车", false);
        ...
        formStack.add(createTransportModeRow(), formGbc);
        ...
        walkingButton.addActionListener(e -> setTransportMode(TransportMode.WALKING));
        shuttleBusButton.addActionListener(e -> setTransportMode(TransportMode.SHUTTLE_BUS));
        ...
        queryButton.addActionListener(e -> {
            if (listener != null) {
                listener.onQuery(selectedStartId(), selectedEndId(), transportMode);
            }
        });
    ...
    private static JTextArea createStatusArea() {
        ...
        textArea.setBackground(UiStyles.PRIMARY_50);
    ...
    private static JLabel createStepBadge(String text) {
        ...
        label.setBackground(UiStyles.SURFACE_SUBTLE);
    ...
    private void refreshMapPickUi() {
        ...
        if (mapPickTarget == MapPickTarget.START) {
            applyStatusStyle(UiStyles.PRIMARY_50, UiStyles.PRIMARY_600, normalizeText(mapStatusOverrideText, DEFAULT_START_PICK_STATUS));
            return;
        }
        if (mapPickTarget == MapPickTarget.END) {
            applyStatusStyle(UiStyles.PRIMARY_50, UiStyles.PRIMARY_600, normalizeText(mapStatusOverrideText, DEFAULT_END_PICK_STATUS));
            return;
        }
        applyStatusStyle(UiStyles.SURFACE_SUBTLE, UiStyles.TEXT_SECONDARY, normalizeText(mapStatusOverrideText, DEFAULT_IDLE_STATUS));
    }
    ...
    public interface Listener {
        void onQuery(String startId, String endId, TransportMode transportMode);
    ...
    private void setTransportMode(TransportMode mode) {
        transportMode = mode;
        boolean isWalking = mode == TransportMode.WALKING;
        styleTransportButton(walkingButton, isWalking);
        styleTransportButton(shuttleBusButton, !isWalking);
    }

    private JPanel createTransportModeRow() {
        JPanel row = UiStyles.softCardPanel(new BorderLayout(0, 6));
        ...
        JLabel label = UiStyles.formLabel("出行方式");
        row.add(label, BorderLayout.NORTH);

        JPanel buttonRow = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 0));
        buttonRow.setOpaque(false);
        buttonRow.add(walkingButton);
        buttonRow.add(shuttleBusButton);
        row.add(buttonRow, BorderLayout.CENTER);

        return row;
    }

    private JButton createTransportModeButton(String text, boolean active) {
        JButton button = new JButton(text);
        button.setFocusPainted(false);
        button.setFont(UiStyles.BODY_FONT);
        button.setPreferredSize(new Dimension(96, 34));
        styleTransportButton(button, active);
    ...
    private static void styleTransportButton(JButton button, boolean active) {
        if (active) {
            button.setBackground(UiStyles.PRIMARY_600);
            button.setForeground(Color.WHITE);
            button.setBorder(BorderFactory.createLineBorder(UiStyles.PRIMARY_600));
        } else {
            button.setBackground(UiStyles.SURFACE);
            button.setForeground(UiStyles.TEXT_SECONDARY);
            button.setBorder(BorderFactory.createLineBorder(UiStyles.BORDER));
        }
    }
}
