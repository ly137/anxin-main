...
import com.anxin.navigation.application.auth.AuthenticationException;
import com.anxin.navigation.application.auth.AuthorizationException;
import com.anxin.navigation.domain.planning.NoRouteFoundException;
...
public final class ControllerExceptionMapper {
    ...
    private static String normalizeFileMessage(Throwable throwable) {
        String deepestMsg = safe(findDeepestMessage(throwable), "");
        String lower = deepestMsg.toLowerCase(Locale.ROOT);
        if (matchesFilePattern(lower)) {
            return fileMessageFor(lower, deepestMsg);
        }
        String chainMsg = safe(findFirstMatchingMessage(throwable), "");
        if (!chainMsg.isEmpty()) {
            String chainLower = chainMsg.toLowerCase(Locale.ROOT);
            if (matchesFilePattern(chainLower)) {
                return fileMessageFor(chainLower, chainMsg);
            }
        }
        return safe(deepestMsg, "文件读写失败，请检查数据目录和权限。");
    }

    private static boolean matchesFilePattern(String lower) {
        return lower.contains("backup not found")
                || lower.contains("required backup file missing")
                || lower.contains("failed to backup data")
                || lower.contains("failed to restore backup")
                || lower.contains("failed to read data file")
                || lower.contains("invalid json format")
                || lower.contains("failed to write data file")
                || lower.contains("data file not found")
                || lower.contains("failed to create data directories");
    }

    private static String fileMessageFor(String lower, String fallback) {
        ...
        return safe(fallback, "文件读写失败，请检查数据目录和权限。");
    }

    private static String findFirstMatchingMessage(Throwable throwable) {
        Throwable cursor = throwable;
        while (cursor != null) {
            if (cursor.getMessage() != null && !cursor.getMessage().trim().isEmpty()) {
                String lower = cursor.getMessage().toLowerCase(Locale.ROOT);
                if (matchesFilePattern(lower)) {
