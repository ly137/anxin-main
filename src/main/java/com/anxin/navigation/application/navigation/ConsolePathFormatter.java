...
public class ConsolePathFormatter {
    ...
    public String format(PathResult pathResult, String transportLabel) {
        ...
        String label = transportLabel == null || transportLabel.trim().isEmpty() ? "步行" : transportLabel.trim();

        StringBuilder builder = new StringBuilder();
        builder.append("========== 路径规划结果 ==========").append(System.lineSeparator());
        builder.append("出行方式：").append(label).append(System.lineSeparator());
        ...
        builder.append("预计").append(label).append("耗时：").append(String.format("%.1f", pathResult.getEstimatedTime())).append(" 分钟").append(System.lineSeparator());
